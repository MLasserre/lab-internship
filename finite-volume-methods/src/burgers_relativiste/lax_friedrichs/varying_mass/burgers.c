...
#define FILENAME_W "lax_friedrichs_w_mass.dat"   
/* Nom du fichier ou on ecrit la solution pour la vitesse*/
#define FILENAME_V "lax_friedrichs_v_mass.dat"
...
int main(void){
	...
	int nt = 0;               /* nombre de points en temps
                                     pour un x donne */
	int n_stop = 0;           /* nombre d'iteration avant arret */

	real M = 0.;              /* Masse de l'objet */
	real dm = 0.;             /* pas en masse */
	real M_f = 0.;            /* Masse finale */
	...
	printf("Donner le nombre d'iteration avant arret n_stop: ");
	scanf("%d", &n_stop);
	printf("n_stop = %d\n", n_stop);

	printf("Entrer la valeur du pas en masse dm: ");
	scan_real(&dm);
	printf("dm = %g\n", dm);

	printf("Donner la masse finale: ");
	scan_real(&M_f);
	printf("M_f = %g\n", M_f);
