/**********************************************************************
 * Nom ............ : burgers.c
 * Role ........... : Resolution de l'equation de Burgers avec la
 *                    methode de Glimm.
...
#define FILENAME "glimm.dat"   /* Nom du fichier ou on
                                  ecrit la solution */
...
int main(void){
    ...
    /* Initialisation de la "graine" pour le tirage de la
       variable aleatoire necessaire a la methode de Glimm */
	...
	printf("Entrer la valeur a droit de la discontinuite u_right: ");
	scan_real(&u_right);
	printf("u_right = %g\n", u_right);
	...
	for(n=1;n<nt;n++){
	//for(n=1;n<5;n++){
		glimm(nx, l_border, r_border, k, h, uold, unew);
