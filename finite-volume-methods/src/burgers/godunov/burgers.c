...
#define FILENAME "godunov.dat"   /* Nom du fichier ou on
                                           ecrit la solution */
...
int main(void){
	...
	remove(FILENAME);           /* Si le fichier existe
                                   deja on le supprime */
	entete(FILENAME, nx-2, nt); /* Ecriture de l'entete
                                 contenant le nombre
                                 de points en x et
                                 d'instant */
	...
	ecrit(FILENAME, nx, uold); /* Ecriture de la solution */
                               /* au temps initial */
	for(n=1;n<nt;n++){
		godunov(nx, alpha, l_border, r_border, uold, unew);
